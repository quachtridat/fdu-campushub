...
const CampusHubLayout: React.FC<PropsType> = ({
  ...
  return (
    <div className={`flex flex-col flex-1 ${propClassName || ''}`} {...otherProps}>
      <CampusHubNavBar
        definedSearchOptions={navDefinedSearchOptions}
        className="fixed top-0 z-50 w-full h-[4rem]"
      />
      {children}
      <div className={`flex flex-col items-center justify-around p-4 bg-white border-t border-black ${footerClassName || ''}`}>
        <p>
          Written by{' '}
